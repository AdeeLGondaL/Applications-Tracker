      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] xl:grid-cols-[9.5rem_minmax(0,0.82fr)_minmax(0,1.22fr)]">
          ...
          <div className="grid gap-0.5">
            ...
            {sidebarNav.map(([icon, label, on]) => (
              <span key={label} className={`flex items-center gap-2.5 rounded-[8px] px-2.5 py-2 text-[13px] font-medium ${on ? "bg-[var(--applume-accent-soft)] text-[var(--applume-accent-hover)]" : "text-[var(--text-muted)]"}`}>
                <Icon name={icon} className="h-3.5 w-3.5" /> {t(`phrases.${label}`)}
              </span>
            ))}
          </div>
        ...
        <div className="border-b border-[var(--border)] lg:border-b-0 lg:border-r">
          <div className="flex items-center justify-between px-4 pt-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--text-soft)]">
            <span>{t(`phrases.${set.identity}`)}</span><span>{set.records.length}</span>
          </div>
          <ul className="p-2">
            {set.records.map((r) => {
                  ...
                  <button type="button" onClick={() => setActiveId(r.id)}
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center justify-between gap-2">
                        ...
                        <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${toneStyles[r.tone]}`}>{t(`phrases.${r.status}`)}</span>
                      </span>
                      <span className="mt-0.5 block truncate text-xs text-[var(--text-muted)]">{r.detail}</span>
                      <span className="mt-1 flex items-center gap-1.5 text-[11px] text-[var(--text-soft)]"><Icon name="calendar" className="h-3 w-3" /> {t(`phrases.${r.deadline}`)}</span>
                    </span>
                  </button>
                </li>
              );
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${toneStyles[active.tone]}`}>{t(`phrases.${active.status}`)}</span>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className={`rounded-[10px] border p-3 ${active.tone === "warning" ? toneStyles.warning : "border-[var(--border)] bg-[var(--surface-soft)]"}`}>
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] opacity-70">{t("phrases.Deadline")}</p>
              <p className="mt-1 text-sm font-semibold">{t(`phrases.${active.deadline}`)}</p>
              <p className="text-[11px] opacity-75">{t(`phrases.${active.deadlineNote}`)}</p>
            </div>
            <div className="rounded-[10px] border border-[var(--border)] bg-[var(--surface-soft)] p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--text-soft)]">{t("phrases.Next action")}</p>
              <p className="mt-1 text-sm font-medium text-[var(--ink)]">{t(`phrases.${active.next}`)}</p>
            </div>
          </div>
          <div className="mt-4">
            <div className="flex items-center justify-between gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--text-soft)]">
              <span>{t(`phrases.${set.docLabel}`)}</span><span className="shrink-0 whitespace-nowrap tabular-nums">{t("phrases.{done}/{total} ready", { done: readyCount, total: active.docs.length })}</span>
            </div>
            <ul className="mt-2 grid gap-1.5">
              {active.docs.map(([label, done]) => (
                <li key={label} className="flex items-center gap-2.5 text-sm">
                  ...
                  <span className={done ? "text-[var(--ink)]" : "text-[var(--text-muted)]"}>{t(`phrases.${label}`)}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="mt-4 rounded-[10px] bg-[var(--surface-soft)] p-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--text-soft)]">{t("phrases.Note")} · {t(`phrases.${active.source}`)}</p>
            <p className="mt-1 text-[13px] leading-6 text-[var(--text-muted)]">{t(`phrases.${active.note}`)}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="mx-auto grid w-full max-w-7xl items-start gap-10 px-4 sm:px-6 lg:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)] lg:gap-12 xl:gap-16">
      <div className="relative z-10">
        <SectionLabel>{t("phrases.Application tracking, reconsidered")}</SectionLabel>
        <h1 className="font-display mt-6 text-[clamp(2.5rem,5.2vw,4.25rem)] leading-[1.02] tracking-[-0.02em] text-[var(--text-strong)]">
          {t("phrases.Every application.")}<br />{t("phrases.One calm place.")}
        </h1>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          <a href="#how-it-works" className="inline-flex min-h-12 items-center gap-2 rounded-[11px] px-4 text-[15px] font-medium text-[var(--ink)] transition-colors hover:text-[var(--applume-accent-hover)]">
            {t("phrases.View product")} <Icon name="download" className="h-4 w-4 rotate-[-90deg]" />
          </a>
        </div>
        <p className="mt-5 text-[13px] text-[var(--text-soft)]">{t("phrases.Free to use · Private by default · Export anytime")}</p>
      </div>
  ...
  return (
    <div className="border-t border-[var(--border)]">
      <button type="button" onClick={() => setOpen((v) => !v)} aria-expanded={open} className="flex w-full items-center justify-between gap-6 py-5 text-left">
        <span className="text-base font-medium text-[var(--ink)]">{t(`phrases.${q}`)}</span>
      ...
      {open && <p className="max-w-2xl pb-5 text-[15px] leading-7 text-[var(--text-muted)]">{t(`phrases.${a}`)}</p>}
    </div>
  );
}
...
function SheetCard() {
  ...
  <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-card)] p-6 shadow-[0_1px_0_rgba(0,0,0,0.02),0_18px_50px_-40px_rgba(12,20,16,0.3)]">
    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--text-soft)]">{t("phrases.A spreadsheet row")}</p>
    <div className="mt-4 overflow-x-auto">
      <table className="w-full text-left text-[13px] text-[var(--text-muted)]">
        <thead className="text-[11px] uppercase tracking-[0.1em] text-[var(--text-soft)]">
          <tr className="border-b border-[var(--border)]"><th className="py-2 pr-3 font-semibold">{t("phrases.Name")}</th><th className="py-2 pr-3 font-semibold">{t("phrases.Status")}</th><th className="py-2 font-semibold">{t("phrases.Date")}</th></tr>
        </thead>
        <tbody>
          <tr className="border-b border-[var(--border)]"><td className="py-2 pr-3">TU Munich</td><td className="py-2 pr-3">{t("phrases.applying?")}</td><td className="py-2">15/6</td></tr>
          <tr className="border-b border-[var(--border)]"><td className="py-2 pr-3">SAP intern</td><td className="py-2 pr-3">{t("phrases.sent")}</td><td className="py-2">—</td></tr>
          <tr><td className="py-2 pr-3">Zalando</td><td className="py-2 pr-3">{t("phrases.todo")}</td><td className="py-2">?</td></tr>
        </tbody>
      </table>
    </div>
    <p className="mt-4 text-[13px] leading-6 text-[var(--text-soft)]">{t("phrases.Documents, links, notes and next steps don't fit — so they end up in other tabs, emails and your memory.")}</p>
  </div>
  );
}
...
function RecordCard() {
  ...
  <div className="rounded-[var(--radius-lg)] border border-[var(--applume-accent-border)] bg-[var(--surface-card)] p-6 shadow-[0_18px_50px_-30px_rgba(0,153,102,0.55)]">
    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--applume-accent-hover)]">{t("phrases.An Applume record")}</p>
    <p className="mt-4 text-base font-semibold text-[var(--ink)]">TU Munich · M.Sc. Computer Science</p>
    <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
      <span className={`rounded-full border px-2 py-0.5 font-semibold ${toneStyles.warning}`}>{t("phrases.Deadline 15 Jun · 9 days")}</span>
      <span className={`rounded-full border px-2 py-0.5 font-semibold ${toneStyles.neutral}`}>{t("phrases.Next: upload transcript")}</span>
    </div>
    <ul className="mt-4 grid gap-1.5 text-sm">
      {[["Curriculum vitae", true], ["Transcript", false], ["Motivation letter", true], ["Portal link saved", true]].map(([l, d]) => (
        <li key={l} className="flex items-center gap-2.5">
          ...
          <span className={d ? "text-[var(--ink)]" : "text-[var(--text-muted)]"}>{t(`phrases.${l}`)}</span>
        </li>
      ))}
    </ul>
  </div>
...
function MorphHeading() {
  ...
  return (
  <>
    <SectionLabel index="01">{t("phrases.From row to record")}</SectionLabel>
    <h2 className="font-display mt-6 max-w-3xl text-[clamp(2rem,4.2vw,3.25rem)] leading-[1.06] tracking-[-0.015em]">
      {t("phrases.A spreadsheet remembers the row.")}<br />
      <span className="text-[var(--applume-accent)]">{t("phrases.Applume remembers the application.")}</span>
    </h2>
  </>
  );
}
    ...
    <div className="relative min-h-dvh bg-[var(--surface)] text-[var(--ink)]">
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-[color-mix(in_srgb,var(--surface)_82%,transparent)] backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          <nav className="hidden items-center gap-8 md:flex">
            {nav.map(([label, href]) => (
              <a key={href} href={href} className="text-sm font-medium text-[var(--text-muted)] transition-colors hover:text-[var(--ink)]">{t(`phrases.${label}`)}</a>
            ))}
          </nav>
      ...
      <main id="main">
        ...
        <section id="how-it-works" className="border-t border-[var(--border)]">
          <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 sm:py-28">
            <Reveal>
              <SectionLabel index="02">{t("phrases.Less data entry")}</SectionLabel>
              <h2 className="font-display mt-6 max-w-3xl text-[clamp(2rem,4.2vw,3.25rem)] leading-[1.06] tracking-[-0.015em]">
                {t("phrases.Paste the opportunity. Review the details. Keep moving.")}
              </h2>
            </Reveal>
            <div className="mt-12 grid gap-px overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--border)] md:grid-cols-3">
              {[
                ...
                <Reveal key={n} delay={i * 0.06} className="bg-[var(--surface-card)] p-7">
                  ...
                  <p className="mt-4 text-lg font-semibold text-[var(--ink)]">{t(`phrases.${title}`)}</p>
                  <p className="mt-2 text-[15px] leading-7 text-[var(--text-muted)]">{t(`phrases.${copy}`)}</p>
                </Reveal>
              ))}
            </div>
          </div>
        ...
        <section id="features" className="border-t border-[var(--border)]">
          <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 sm:py-28">
            <Reveal>
              <SectionLabel index="03">{t("phrases.The core system")}</SectionLabel>
              <h2 className="font-display mt-6 max-w-3xl text-[clamp(2rem,4.2vw,3.25rem)] leading-[1.06] tracking-[-0.015em]">
                {t("phrases.Four things a spreadsheet can't keep together.")}
              </h2>
            </Reveal>
            <div className="mt-10">
              {coreSystem.map((f, i) => (
                <Reveal key={f.n} delay={i * 0.03} amount={0.5}>
                  <div className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-2 border-t border-[var(--border)] py-8 sm:grid-cols-[5rem_1fr_1fr] sm:gap-x-10">
                    ...
                    <h3 className="text-xl font-semibold tracking-[-0.01em] text-[var(--ink)]">{t(`phrases.${f.title}`)}</h3>
                    <p className="col-span-2 text-[15px] leading-7 text-[var(--text-muted)] sm:col-span-1 sm:col-start-3 sm:row-start-1">{t(`phrases.${f.copy}`)}</p>
                  </div>
                </Reveal>
              ))}
            </div>
            ...
            <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
              <Reveal>
                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-x-4 text-sm">
                  <div className="pb-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--applume-accent-hover)]">{t("phrases.University")}</div>
                  <div />
                  <div className="pb-3 text-right text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--info)]">{t("phrases.Job")}</div>
                  {vocab.map(([, u, j]) => (
                    <div key={u} className="contents">
                      <div className="border-t border-[var(--border)] py-3.5 font-medium text-[var(--ink)]">{t(`phrases.${u}`)}</div>
                      ...
                      <div className="border-t border-[var(--border)] py-3.5 text-right font-medium text-[var(--ink)]">{t(`phrases.${j}`)}</div>
                    </div>
                  ))}
                </div>
              </Reveal>
              <Reveal delay={0.06} className="lg:pl-4">
                <SectionLabel index="04">{t("phrases.One system, two worlds")}</SectionLabel>
                <h2 className="font-display mt-6 text-[clamp(2rem,4.2vw,3.25rem)] leading-[1.06] tracking-[-0.015em]">
                  {t("phrases.University and jobs, in the same calm structure.")}
                </h2>
                <p className="mt-6 max-w-md text-[15px] leading-7 text-[var(--text-muted)]">
                  {t("phrases.The structure never changes — only the words do. Switch a record between a university application and a job application and the labels follow.")}
                </p>
              </Reveal>
            </div>
          </div>
        ...
        <section id="faq" className="border-t border-[var(--border)]">
          <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 sm:py-28">
            <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16">
              <Reveal>
                <SectionLabel index="05">{t("phrases.Frequently asked")}</SectionLabel>
                <h2 className="font-display mt-6 text-[clamp(2rem,4.2vw,3rem)] leading-[1.06] tracking-[-0.015em]">{t("phrases.Questions, answered.")}</h2>
                <p className="mt-6 max-w-sm text-[15px] leading-7 text-[var(--text-muted)]">
                  {t("phrases.A few things people ask before moving their applications over.")}
                </p>
                <p className="mt-4 max-w-sm text-[15px] leading-7 text-[var(--text-muted)]">
                  {t("phrases.Anything else on your mind?")}{" "}
                  <a href="mailto:hello@applume.app" className="font-medium text-[var(--applume-accent-hover)] hover:underline">{t("phrases.Email us")}</a> — {t("phrases.we read every message.")}
                </p>
              </Reveal>
              <Reveal delay={0.06}>
    ...
    <footer className="border-t border-[var(--border)] bg-[var(--surface)]">
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <div className="grid gap-10 md:grid-cols-[1.6fr_1fr_1fr]">
          <div>
            <Logo />
            <p className="mt-4 max-w-xs text-sm leading-6 text-[var(--text-muted)]">{t("phrases.One calm workspace for university and job applications — deadlines, documents, notes and next steps included.")}</p>
            <p className="mt-6 text-sm font-semibold text-[var(--ink)]">{t("phrases.Know someone still stuck in a spreadsheet?")}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {canNativeShare && (
                <button type="button" onClick={handleNativeShare} className={pill}>
                  <Icon name="share" className="h-3.5 w-3.5" /> {t("phrases.Share")}
                </button>
              )}
              <button type="button" onClick={handleCopy} className={pill}>
                <Icon name={copied ? "check" : "copy"} className="h-3.5 w-3.5" /> {copied ? t("phrases.Link copied") : t("phrases.Copy link")}
              </button>
          ...
          <div>
            <p className={heading}>{t("phrases.Guides")}</p>
            <ul className="mt-4 grid gap-2.5">
              <li><a className={link} href="/university-application-tracker">{t("phrases.University application tracker")}</a></li>
              <li><a className={link} href="/huntr-alternative">{t("phrases.Huntr alternative")}</a></li>
              <li><a className={link} href="/teal-alternative">{t("phrases.Teal alternative")}</a></li>
            </ul>
          </div>
          <div>
            <p className={heading}>{t("phrases.Company")}</p>
            <ul className="mt-4 grid gap-2.5">
              <li><a className={link} href="mailto:hello@applume.app">{t("phrases.Contact")}</a></li>
              <li><a className={link} href="/privacy">{t("phrases.Privacy")}</a></li>
              <li><a className={link} href="/terms">{t("phrases.Terms")}</a></li>
            </ul>
          </div>
        </div>
        <div className="mt-12 flex flex-col items-start justify-between gap-3 border-t border-[var(--border)] pt-6 text-xs text-[var(--text-soft)] sm:flex-row sm:items-center">
          <p>© {new Date().getFullYear()} Applume. {t("phrases.Applications deserve better than a spreadsheet.")}</p>
          <p>{t("phrases.Private to your account · Export anytime")}</p>
        </div>
      </div>
    </footer>
  );
