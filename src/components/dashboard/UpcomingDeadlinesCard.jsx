      ...
      <CardContent className="p-4 sm:p-5">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500 dark:text-[#71717a]">Top records that need time-sensitive action.</p>
          </div>
            ...
            {apps.map((app, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="rounded-xl border border-slate-100 bg-slate-50 p-3 dark:border-[#2a2a2e] dark:bg-[#111113]"
                >
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold leading-tight text-slate-900 dark:text-white">{app.name}</p>
                      <p className="mt-1 truncate text-xs text-slate-500 dark:text-[#71717a]">{app.programRole || app.type}</p>
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <p className="text-[11px] font-semibold text-slate-500 dark:text-[#71717a]">{formatDate(app.deadline)}</p>
                    {onOpenRecord && (
                      <button
                        type="button"
