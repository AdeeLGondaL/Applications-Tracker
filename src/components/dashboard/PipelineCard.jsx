      ...
      <CardContent className="p-4 sm:p-5">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500 dark:text-[#9AA4B2]">{total} records grouped into four phases.</p>
          </div>
          {activeRows.length > 0 && (
            <button
              type="button"
              onClick={() => setExpanded((value) => !value)}
              className="shrink-0 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-600 transition hover:border-[var(--applume-accent-border)] hover:bg-[var(--applume-accent-soft)] hover:text-[var(--applume-accent-hover)] dark:border-[rgba(255,255,255,0.09)] dark:bg-[#20242A] dark:text-[#9AA4B2]"
            >
        ...
        {total === 0 ? (
          <p className="py-6 text-center text-sm text-slate-400 dark:text-[#9AA4B2]">Add records to see your pipeline take shape.</p>
        ) : (
          <div>
            <div className="flex h-3 overflow-hidden rounded-full bg-slate-100 dark:bg-[#20242A]" aria-label="Application pipeline summary">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-4">
              {segments.map(({ label, count, color }) => {
                ...
                return (
                  <div key={label} className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 dark:border-[rgba(255,255,255,0.09)] dark:bg-[#20242A]">
                    <div className="flex items-center gap-2">
                      <span className={`h-2 w-2 rounded-full ${color}`} />
                      <span className="text-xs font-bold text-slate-700 dark:text-[#d4d4d8]">{label}</span>
                    </div>
                    <p className="mt-1 text-xs text-slate-500 dark:text-[#9AA4B2]">
              ...
              return (
                <div key={status} className="sm:flex sm:items-center sm:gap-3">
                  <div className="mb-1.5 flex min-w-0 items-center justify-between gap-3 sm:mb-0 sm:w-36 sm:shrink-0">
                    <div className="min-w-0 truncate text-xs font-semibold text-slate-500 dark:text-[#9AA4B2]">{status}</div>
