    ...
    return (
      <Card className="rounded-[var(--radius-lg)] border border-dashed border-[var(--border-strong)] bg-[var(--surface-card)]">
        <CardContent className="grid place-items-center p-12 text-center">
          ...
          <h3 className="mt-4 font-display text-lg font-semibold text-[var(--text-strong)]">No linked documents yet</h3>
          <p className="mt-2 max-w-md text-sm leading-6 text-[var(--text-muted)]">
            Attach a link (Google Drive, Dropbox, ...) to any document in an application's checklist and it will
            appear here — ready to reuse on your next application.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-card)] shadow-[0_1px_0_rgba(0,0,0,0.02),0_18px_50px_-40px_rgba(12,20,16,0.28)]">
      <CardContent className="p-0">
        <div className="border-b border-[var(--border)] px-5 py-4">
          <h2 className="font-display text-lg font-semibold leading-tight text-[var(--text-strong)]">Your documents</h2>
          <p className="mt-1 text-[13px] leading-5 text-[var(--text-muted)]">
            {library.length} linked document{library.length === 1 ? "" : "s"} across your applications. Reuse them from
            any checklist via "From your documents".
          </p>
        </div>
            ...
            <li key={doc.url} className="flex min-w-0 items-center gap-3.5 px-5 py-3.5 transition-colors hover:bg-[var(--surface-soft)]">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 truncate text-[13px] leading-5 text-[var(--text-muted)]">
                  via {doc.sourceName}
                  {doc.count > 1 && <span className="text-[var(--text-soft)]"> · used in {doc.apps.length} application{doc.apps.length === 1 ? "" : "s"}</span>}
                </p>
              </div>
              <button
                type="button"
