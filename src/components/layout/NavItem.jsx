...
export function NavItem({ icon, label, count, active = false, alert = false, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group w-full flex items-center justify-between rounded-xl px-3 py-2 text-sm font-semibold transition-colors ${
        active
        ...
        <span
          className={`rounded-full px-2 py-0.5 text-xs font-bold ${
            active
              ...
              : "bg-slate-100 text-slate-500 dark:bg-[#1c1c1f] dark:text-[#71717a]"
          }`}
        >
          {count}
        </span>
