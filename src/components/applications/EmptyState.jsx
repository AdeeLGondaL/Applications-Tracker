  ...
  return (
    <Card className="rounded-[var(--radius-lg)] border border-dashed border-[var(--border-strong)] bg-[var(--surface-card)]">
      <CardContent className="grid place-items-center p-12 text-center">
        ...
        <h3 className="mt-4 font-display text-lg font-semibold text-[var(--text-strong)]">{t("phrases.No matching records")}</h3>
        <p className="mt-2 max-w-md text-sm leading-6 text-[var(--text-muted)]">
          {t("phrases.Clear filters or add a new application record to keep building your tracker.")}
        </p>
      </CardContent>
    </Card>
  );
...
export function EmptyDashboard({ onAdd, onImport }) {
  ...
  return (
    ...
    >
      ...
      <h2 className="font-display text-2xl font-semibold text-[var(--text-strong)]">{t("phrases.Track your first application")}</h2>
      <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-[var(--text-muted)]">
        {t("phrases.Add one real application or import an existing backup. Applume will keep deadlines, links, notes, documents, and next steps together.")}
      </p>
